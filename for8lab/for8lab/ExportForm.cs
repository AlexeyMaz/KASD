    ...
    public partial class ExportForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (rbExcel.Checked == true)
            {
                ...
                {
                    customersViewTableAdapter1.Fill(sqL_ShippingDataSet1.CustomersView);
                    ExportToExcel(sqL_ShippingDataSet1.CustomersView);
                }
                if (checkBox2.Checked)
                {
                    cargoesViewTableAdapter1.Fill(sqL_ShippingDataSet1.CargoesView);
                    ExportToExcel(sqL_ShippingDataSet1.CargoesView);
                }
            }
            if (rbHTML.Checked == true)
            {
                ...
                {
                    customersViewTableAdapter1.Fill(sqL_ShippingDataSet1.CustomersView);
                    ExportToHTML(sqL_ShippingDataSet1.CustomersView, checkBox1.Text);
                }
                if (checkBox2.Checked)
                {
                    cargoesViewTableAdapter1.Fill(sqL_ShippingDataSet1.CargoesView);
                    ExportToHTML(sqL_ShippingDataSet1.CargoesView, checkBox2.Text);
                }
            }
        }
    }
